...
const CATEGORIES = [
  { name: "Prize Money", nameTh: "ค่าเงินรางวัลการแข่งขัน", icon: "ph-trophy", docsPre: [MASTER_DOCS[0], MASTER_DOCS[1], MASTER_DOCS[2], MASTER_DOCS[3], MASTER_DOCS[4], MASTER_DOCS[5]], notes: "" },
  { name: "Judge Compensation", nameTh: "ค่าตอบแทนกรรมการตัดสิน", icon: "ph-gavel", docsPre: [MASTER_DOCS[6], MASTER_DOCS[0], MASTER_DOCS[7], MASTER_DOCS[2], MASTER_DOCS[3], MASTER_DOCS[8], MASTER_DOCS[9]], notes: "" },
  { name: "Hotel Accommodation", nameTh: "ค่าห้องพักโรงแรมสำหรับผู้เข้าร่วม", icon: "ph-bed", docsPre: [MASTER_DOCS[10], MASTER_DOCS[11]], notes: "" },
  { name: "Catering (Food)", nameTh: "ค่าจัดเลี้ยง (อาหาร)", icon: "ph-fork-knife", docsPre: [MASTER_DOCS[10], MASTER_DOCS[11]], notes: "" },
  { name: "Snacks / Refreshments", nameTh: "ค่าอาหารว่าง", icon: "ph-coffee", docsPre: [MASTER_DOCS[11]], notes: "ตรวจสอบ: ชื่อลูกค้าต้องเป็นคณะเภสัชศาสตร์ จุฬาลงกรณ์มหาวิทยาลัย · ที่อยู่ 254 ถ.พญาไท แขวงวังใหม่ เขตปทุมวัน กรุงเทพฯ 10330 · เลขผู้เสียภาษี 0994000158734" },
  { name: "Staff Travel", nameTh: "ค่าเดินทางสำหรับผู้ปฏิบัติงาน", icon: "ph-airplane-tilt", docsPre: [MASTER_DOCS[11]], notes: "" },
  { name: "Beverage Booth", nameTh: "ค่าบูธเครื่องดื่ม", icon: "ph-storefront", docsPre: [MASTER_DOCS[10], MASTER_DOCS[11]], notes: "" },
  { name: "Museum Entry Fee", nameTh: "ค่าธรรมเนียมเข้าพิพิธภัณฑ์", icon: "ph-ticket", docsPre: [MASTER_DOCS[11]], notes: "กรณียังไม่มี vendor กับจุฬา: แนบแบบฟอร์มรายละเอียดผู้ขาย" },
  { name: "Science Officer Compensation", nameTh: "ค่าตอบแทนเจ้าหน้าที่วิทยาศาสตร์", icon: "ph-flask", docsPre: [MASTER_DOCS[6], MASTER_DOCS[0], MASTER_DOCS[8]], notes: "" },
];
...
export async function seedDemo(prisma, roleIds) {
  ...
  // Demo txns double as the accounts' opening history — each one's amount is applied
  // to its account's balance below, so the balance always reconciles exactly with the
  // ledger (never a hardcoded balance disconnected from the txn history).
  const now = Date.now();
  const demoTxns = [
  ...
  await prisma.txn.createMany({ data: demoTxns });
  for (const t of demoTxns) {
    await prisma.account.update({
      where: { id: t.acctId },
      data: { balance: { increment: t.type === "in" ? t.amount : -t.amount } },
    });
  }

  const cats = await prisma.category.findMany();
  const cat = (name) => cats.find((c) => c.name === name);
  const mkdocs = (c, filled) => c.docsPre.map((d, i) => ({ name: d, phase: "pre", submitted: i < filled, link: i < filled ? "https://drive.google.com/file/d/demo-" + i + "/view" : null, fileName: null, disc: null }));
