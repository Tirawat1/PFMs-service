...
export async function reverseRequestTx(prisma, {
  ...
  facultyAcctId, projectAcctId, projectionId,
}) {
  return prisma.$transaction(async (tx) => {
    ...
    if (clearsDisbursement) {
      ...
      if (refundAmount > 0) {
        await tx.account.update({ where: { id: facultyAcctId }, data: { balance: { decrement: refundAmount } } });
        await tx.account.update({ where: { id: projectAcctId }, data: { balance: { increment: refundAmount } } });
        await tx.txn.create({ data: { acctId: facultyAcctId, type: "out", amount: refundAmount, desc: "Reversal (advance return undone) — " + id } });
        await tx.txn.create({ data: { acctId: projectAcctId, type: "in", amount: refundAmount, desc: "Reversal (advance return undone) — " + id } });
      }
    }
