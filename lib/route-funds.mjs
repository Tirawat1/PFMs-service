...
export async function routeFundsTx(prisma, { reqId, streamId, streamAcctId, amount, facultyAcctId, title, by, byRole }) {
  return prisma.$transaction(async (tx) => {
    ...
    await tx.txn.create({ data: { acctId: facultyAcctId, type: "out", amount, desc: "Fund routing — " + title } });
    await tx.txn.create({ data: { acctId: streamAcctId, streamId, type: "in", amount, desc: "Fund routing — " + title } });
